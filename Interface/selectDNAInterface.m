...
function handles = createInterface(handles)
    ...
    % slider
    [~, handles.dna.autoDnaBinaryThresholdSlider] = javacomponent('javax.swing.JSlider');
    handles.dna.autoDnaBinaryThresholdSlider.set('Parent', handles.dna.autoDnaBinaryThresholdBox);
    handles.dna.autoDnaBinaryThresholdSlider.JavaPeer.set('Maximum', 1e6);
    handles.dna.autoDnaBinaryThresholdSlider.JavaPeer.set('Minimum', 0);
    handles.dna.autoDnaBinaryThresholdSlider.JavaPeer.set('Value', 5e5);
    handles.dna.autoDnaBinaryThresholdSlider.JavaPeer.set('MouseReleasedCallback',...
                                            @(~,~) setAutoDnaBinaryThreshold(handles.dna.autoDnaBinaryThresholdSlider,...
                                            get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Value')/get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Maximum')...
                                            ));
    % texbox
    handles.dna.autoDnaBinaryThresholdTextBox = uicontrol(  'Parent', handles.dna.autoDnaBinaryThresholdBox,...
                                                            'Style','edit',...
                                                            'String', '0.5',...
                                                            'Callback', @(hObject,~) setAutoDnaBinaryThreshold(hObject,str2double(hObject.String)));
    % widths
    handles.dna.autoDnaBinaryThresholdBox.set('Widths',[-1 50]);
    ...
    % box
    handles.dna.autoDnaLengthBox = uix.HBox('Parent', handles.dna.autoDNABox);
    % slider (this must come first)
    [~, handles.dna.autoDnaLengthSlider] = javacomponent('com.jidesoft.swing.RangeSlider');
    % min textbox
    handles.dna.autoDnaLengthMinTextBox = uicontrol('Parent', handles.dna.autoDnaLengthBox,...
                                                    'Style','edit',...
                                                    'String', '10',...
                                                    'Callback', @(hObject,~) ...
                                                        setAutoDnaLength(hObject,...
                                                        str2double(hObject.String),...
                                                        get(handles.dna.autoDnaLengthSlider.JavaPeer,'HighValue')/get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum')*100 ...
                                                        ));
    % slider settings
    handles.dna.autoDnaLengthSlider.set('Parent', handles.dna.autoDnaLengthBox);
    handles.dna.autoDnaLengthSlider.JavaPeer.set('Maximum', 100e6);
    handles.dna.autoDnaLengthSlider.JavaPeer.set('Minimum', 0);
    handles.dna.autoDnaLengthSlider.JavaPeer.set('LowValue', 10e6);
    handles.dna.autoDnaLengthSlider.JavaPeer.set('HighValue', 100e6);
    handles.dna.autoDnaLengthSlider.JavaPeer.set('MouseReleasedCallback', @(~,~) ...
                                                        setAutoDnaLength(handles.dna.autoDnaLengthSlider,...
                                                        get(handles.dna.autoDnaLengthSlider.JavaPeer,'LowValue')/get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum')*100,...
                                                        get(handles.dna.autoDnaLengthSlider.JavaPeer,'HighValue')/get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum')*100 ...
                                                        ));
    % max textbox
    handles.dna.autoDnaLengthMaxTextBox = uicontrol('Parent', handles.dna.autoDnaLengthBox,...
                                                    'Style','edit',...
                                                    'String', '100',...
                                                    'Callback', @(hObject,~) ...
                                                        setAutoDnaLength(hObject,...
    ...
    % widths
    handles.dna.autoDnaLengthBox.set('Widths',[50 -1 50]);
    ...
    % slider
    [~, handles.dna.autoDnaMinEccentricitySlider] = javacomponent('javax.swing.JSlider');
    handles.dna.autoDnaMinEccentricitySlider.set('Parent', handles.dna.autoDnaMinEccentricityBox);
    handles.dna.autoDnaMinEccentricitySlider.JavaPeer.set('Maximum', 1e6);
    handles.dna.autoDnaMinEccentricitySlider.JavaPeer.set('Minimum', 0);
    handles.dna.autoDnaMinEccentricitySlider.JavaPeer.set('Value', 5e5);
    handles.dna.autoDnaMinEccentricitySlider.JavaPeer.set('MouseReleasedCallback',...
                                            @(~,~) setAutoDnaMinEccentricity(handles.dna.autoDnaMinEccentricitySlider,...
                                            get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Value')/get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Maximum')...
                                            ));
    % texbox
    handles.dna.autoDnaMinEccentricityTextBox = uicontrol(  'Parent', handles.dna.autoDnaMinEccentricityBox,...
                                                            'Style','edit',...
                                                            'String', '0.5',...
                                                            'Callback', @(hObject,~) setAutoDnaMinEccentricity(hObject,str2double(hObject.String)));
    % widths
    handles.dna.autoDnaMinEccentricityBox.set('Widths',[-1 50]);
    ...
    % Heights             
    handles.dna.autoDNABox.set('Heights',[15 20 15 20 15 20 25]);
    ...
    %% 
    handles.dna.leftPanel.set('Heights',[25 200 165 80 60]);
end
...
function handles = loadFromSession(hObject,handles,session)
    ...
    % auto dna settings
    setAutoDnaBinaryThreshold(hObject, session.dna_autoDnaBinaryThreshold);
    setAutoDnaLength(hObject, session.dna_autoDnaMinLength, session.dna_autoDnaMaxLength);
    setAutoDnaMinEccentricity(hObject, session.dna_autoDnaMinEccentricity);
...
function autoDetectDNA(hObject, handles)
    ...
    binaryThreshold = get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Value') / get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Maximum');
    minLength = get(handles.dna.autoDnaLengthSlider.JavaPeer,'LowValue') / get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum') * 100;
    maxLength = get(handles.dna.autoDnaLengthSlider.JavaPeer,'HighValue') / get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum') * 100;
    minEccentricity = get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Value') / get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Maximum');
    ...
    validIds = find([stats.Eccentricity] >= minEccentricity & [stats.MajorAxisLength] >= minLength & [stats.MajorAxisLength] <= maxLength); 
...
function setAutoDnaBinaryThreshold(hObject, value)
    handles = guidata(hObject);
    
    value = value * get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Maximum');
    
    set(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Value', value);
    
    value = get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Value') / get(handles.dna.autoDnaBinaryThresholdSlider.JavaPeer,'Maximum');
    handles.dna.autoDnaBinaryThresholdTextBox.String = num2str(value);
end

function setAutoDnaLength(hObject, low, high)
    handles = guidata(hObject);
    
    low = low * get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum') / 100;
    high = high * get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum') / 100;
    
    set(handles.dna.autoDnaLengthSlider.JavaPeer,'LowValue', low);
    set(handles.dna.autoDnaLengthSlider.JavaPeer,'HighValue', high);
    
    low = get(handles.dna.autoDnaLengthSlider.JavaPeer,'LowValue') / get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum') * 100;
    high = get(handles.dna.autoDnaLengthSlider.JavaPeer,'HighValue') / get(handles.dna.autoDnaLengthSlider.JavaPeer,'Maximum') * 100;
    
    handles.dna.autoDnaLengthMinTextBox.String = num2str(low);
    handles.dna.autoDnaLengthMaxTextBox.String = num2str(high);
end

function setAutoDnaMinEccentricity(hObject, value)
    handles = guidata(hObject);
    
    value = value * get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Maximum');
    
    set(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Value', value);
    
    value = get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Value') / get(handles.dna.autoDnaMinEccentricitySlider.JavaPeer,'Maximum');
    handles.dna.autoDnaMinEccentricityTextBox.String = num2str(value);
end
