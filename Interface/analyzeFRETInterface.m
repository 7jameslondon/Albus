...
function handles = createInterface(handles)
    ...
    setappdata(handles.f,'trace_groupHandles',cell(0,5));
            
    handles.tra.group.grid.set('Heights',25);
    handles.tra.group.grid.set('Widths',[-1 -1 -1 -1 -1]);
    ...
    %% 
    handles.tra.leftPanel.set('Heights',[25 200 80 75 203 250]);
...
function handles = loadFromSession(hObject,handles,session)
    ...
    %% groups
    % remove old groups
    delete(handles.tra.group.grid.Children(6:size(handles.tra.group.grid.Children,1)));
    setappdata(handles.f,'trace_groupHandles',cell(0,5));
    ...
    for i = 1:size(session.tra_groups,1)
        ...
        groupHandles{i,1}.set('String', session.tra_groups{i,1});
...
function updateDisplay(hObject,handles,videoOnlyFlag)
    ...
    if ~exist('videoOnlyFlag','var') % this is excluded when playing the video        
        ...
        for i = 1:numGroups
            groupHandles{i,3}.set('Value',traces.Groups(c,i));
        end
        setappdata(handles.f,'trace_groupHandles',groupHandles);
...
function saveTraceGroups(hObject,handles)
    ...
    for i = 1:numGroups
        traces.Groups(c,i) = groupHandles{i,3}.get('Value');
    end
    
    % update the group counts
    for i = 1:numGroups
        groupHandles{i,2}.set('String', num2str(sum(traces.Groups(:,i))));
    end
...
function addGroup(hObject,handles,loadingFlag)
    ...
    numOrderedObj = size(handles.tra.group.grid.Contents,1)-5;
    groupSeq = [reshape(1:numOrderedObj,numOrderedObj/5,5); numOrderedObj+1:numOrderedObj+5];
...
function deleteGroup(hObject,rowNum)
    ...
    for i = rowNum:numRows
        groupHandles{i, 5}.set('Callback',@(gObject,~) deleteGroup(gObject,i));
    end
