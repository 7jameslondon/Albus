function X_tps = evaluateTPS(X,W,A)
    ...
    for i=1:size(X,1)
        ...
        X_tps(i,1) = W(end-2,1) + W(end-1,1)*X(i,1) + W(end,1)*X(i,2) + sum( W(1:end-3,1) .* energyTPS(mag(A,X(i,:))) ,1);
        % calculate the y corrdinate
        X_tps(i,2) = W(end-2,2) + W(end-1,2)*X(i,1) + W(end,2)*X(i,2) + sum( W(1:end-3,2) .* energyTPS(mag(A,X(i,:))) ,1);
    end
