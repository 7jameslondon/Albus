function tracks = nearestDNANeighborTracking( particlesByFrame, maxLRMovment, maxUDMovment )
        ...
        for currentParticle = 1:size(particlesByFrame{currentFrame},1)
            ...
            if size(particlesByFrame{currentFrame}(currentParticle,:),2) ~= 4 || particlesByFrame{currentFrame}(currentParticle,4) ~= 1
                ...
                for futureFrame = currentFrame+1:size(particlesByFrame,1)
                    if size(particlesByFrame{futureFrame},1) == 0 % no particles in frame
                        break;
                    end
                    XYDistancesToParticles = abs( tracks(iParticle).positions.centers(end,:) - particlesByFrame{futureFrame}(:,1:2) );
                    validParts = XYDistancesToParticles(:,1) <= maxLRMovment & XYDistancesToParticles(:,2) <= maxUDMovment;
                    XYDistancesToParticles(~validParts,:) = inf*ones(sum(~validParts),2);
                    distancesToParticles = sqrt( sum( ( XYDistancesToParticles ).^2 , 2 ) );
                    [closestDistance, closestParticle] = min(distancesToParticles);
                    if validParts(closestParticle)
